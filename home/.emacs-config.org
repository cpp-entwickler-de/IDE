...
(use-package dashboard
  :after helm-buffers
  :custom
  (dashboard-startup-banner (let ((logo (expand-file-name "logo.png" "~/")))
...
(defun cpped-powerline-project-id (icon-face)
  ...
  (when buffer-location
      (let ((project-name (if (and (ignore-errors (projectile-project-p))
        ...
        (concat
         (propertize (all-the-icons-faicon5 (cond ((cpped-tramp-file-remote-p buffer-location) "cloud")
                                                  ((not (= 0 (length project-name))) "folder")
...
*** IEdit Info
#+BEGIN_SRC emacs-lisp
(defun cpped-powerline-iedit-info (icon-face)
  (when iedit-mode
    (format "%s %s/%s"
            (propertize (all-the-icons-faicon5 "pencil-alt")
                        'face (list ':family (all-the-icons-faicon5-family)
                                    ':background (face-attribute icon-face :background))
                        'display '(raise -0.0))
                  ...
                  (:eval
                   (let* ((active (powerline-selected-window-active))
                          ...
                          (lhs (list
                                ...
                                (powerline-raw (cpped-powerline-remote (symbol-value 'face2)) face2 'r)
                                ...
                                (powerline-raw (cpped-powerline-iedit-info (symbol-value 'face2)) face2 'l)
...
(defun cpped-powerline-eshell-status ()
  ...
  (when (and (equal major-mode 'eshell-mode)
    ...
    (concat (propertize (all-the-icons-faicon5 "exclamation-triangle")
                                             'face `(:family ,(all-the-icons-faicon5-family)
...
(use-package didyoumean
  :hook (after-init . didyoumean-mode))
#+END_SRC
...
(add-hook 'find-file-hook (lambda()
                            (setq cpped-auto-format-enabled-p (not (cpped-tramp-file-remote-p (cpped-buffer-name))))))
#+END_SRC

****** Auto-format
#+BEGIN_SRC emacs-lisp
(add-hook 'before-save-hook #'(lambda()
                                (when (and (not (cpped-tramp-file-remote-p (cpped-buffer-name)))
                                           cpped-auto-format-enabled-p
                                           cpped-format-buffer-function)
...
(use-package comment-or-uncomment-sexp
  :quelpa (comment-or-uncomment-sexp
           :fetcher github
...
***** Icons for changes
#+BEGIN_SRC emacs-lisp
(defvar prettify-magit-alist (list (list (rx (= 7 (in "0-9a-f")) space (+ (in space punctuation "*" "/" "\\" "|")) (group "|"))
                                         "│")
                                   (list (rx (= 7 (in "0-9a-f")) space (+ (in space punctuation "*" "/" "\\" "|")) (group "/"))
                                         "╱")
                                   (list (rx (= 7 (in "0-9a-f")) space (+ (in space punctuation "*" "/" "\\" "|")) (group "\\"))
                                         "╲")
                                   (list (rx (= 7 (in "0-9a-f")) space (+ (in space punctuation "*" "/" "\\" "|")) (group "*"))
                                         "┝")
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "add" (? (or "s" "ed"))) eow)
                                         (all-the-icons-faicon5 "plus")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "allow" (? (or "s" "ed"))) eow)
                                         (all-the-icons-faicon5 "thumbs-up")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group (or "remove" "delete") (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "trash")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group (? "bug") "fix" (? (or "up" (group "e" (in "sd"))))) eow)
                                         (all-the-icons-faicon5 "bug")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "call" (? (or "s" "ed"))) eow)
                                         "↴")
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "fixup" (? "!")))
                                         (all-the-icons-faicon5 "wrench")
                                         `(:family ,(all-the-icons-faicon5-family) :foreground ,(face-attribute 'warning :foreground)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "clean" (? (or "s" "ed")) (? (? " ") "up")) eow)
                                         (all-the-icons-faicon5 "paint-brush")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "update" (? (in "sd"))) eow)
                                         "⇪")
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "sync" (? (or "s" "ed"))) eow)
                                         (all-the-icons-faicon5 "sync")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "refactor" (? (or "s" "ed"))) eow)
                                         (all-the-icons-faicon5 "industry")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group (or "replace" "exchange") (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "exchange-alt")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "import" (? (or "s" "ed"))) eow)
                                         (all-the-icons-faicon5 "cart-plus")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "increase" (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "chevron-up")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "decrease" (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "chevron-down")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group (or "enable" "activate") (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "toggle-on")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group (or "disable" "deactivate") (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "toggle-off")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "move" (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "arrow-right")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "optimize" (? (in "sd"))) eow)
                                         (all-the-icons-faicon5 "forward")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "set" (? "s")) eow)
                                         "="
                                         '(:weight 'black :height 1.3))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "show" (? (in "s"))) eow)
                                         (all-the-icons-faicon5 "eye")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "hide" (? (in "s"))) eow)
                                         (all-the-icons-faicon5 "eye-slash")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "lock" (? (or "s" "ed"))) eow)
                                         (all-the-icons-faicon5 "lock")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "unlock" (? (or "s" "ed"))) eow)
                                         (all-the-icons-faicon5 "lock-open")
                                         `(:family ,(all-the-icons-faicon5-family)))
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "use" (? (or "s" "ed"))) eow)
...
Settings                       ^^Snippets            ^^Abbreviations             ^^Packages
  _c_: Open config file            _Y_: New snippet      _A_: New abbreviation       _p_: Emacs packages
  _m_: Open customization file     _y_: Edit snippet     _a_: Edit abbreviations     _P_: system packages
  _t_: Edit theme
...
Tools                      ^^Last Command    ^^System
  ...
  _e_: Evaluate expression                     ^^_s_: Show system services
                                             ^^^^_P_: Manage system packages
  _/_: Open directory                          ^^_d_: Docker
                                             ^^^^_t_: Trash
